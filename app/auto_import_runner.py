...
class Runner:
    def __init__(self, root=ROOT, *, dry_run=False, process_images=False, run_dir=None,
        ...
        self.version = rules_version(self.root / ".codex/skills/1688-woocommerce-profit-import") + ":" + digest(
    ...
    def process(self, task):
        ...
        try:
            ...
            content["category"] = select_category(tree, content["facts"], pack.get("category_terms", []))
            def image_stage():
...
def main():
    ...
    runner = Runner(args.root, dry_run=args.dry_run, process_images=args.process_images,
                    run_dir=args.run_dir, keyword_api=args.keyword_api, draft=args.draft, poll_seconds=args.poll_seconds)
    if args.dry_run:
