...
- Create only verified Fasteners category chains when necessary.
...
Backend publication gate: save a manually observed, current, product-bound `backend-source-acceptance.json` beside the upload checkpoint with site_url, product_id, offer_id, source_url, fingerprint, model_visible=true, source_link_clicked=true, status=PASS. This is evidence of inspecting the existing backend source panel, not authorization to change global templates. Never fabricate the record from REST results. Without it, keep Draft/review; with it and all other gates passed, publish normally.
